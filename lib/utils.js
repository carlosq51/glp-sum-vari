...
/**
 * inicioDiaPeruISO_ — el instante UTC en que empieza (00:00 hora Perú) el día
 * "YYYY-MM-DD", corrido `masDias` días. Sirve para filtrar un timestamptz por
 * día civil peruano: [inicio(d), inicio(d, 1)) es el día d entero.
 *
 * El desfase se lee de Intl a mediodía de ese día en vez de escribir "-05:00":
 * mismo motivo que arriba.
 */
export function inicioDiaPeruISO_(fecha, masDias = 0) {
  const [a, m, d] = String(fecha).split("-").map(Number);
  const mediodiaUtc = new Date(Date.UTC(a, m - 1, d + masDias, 12));
  const horaPeru = Number(new Intl.DateTimeFormat("en-GB", {
    timeZone: TZ_PERU, hour: "2-digit", hourCycle: "h23",
  }).format(mediodiaUtc));
  // A las 12:00 UTC en Lima son las 07:00 → la medianoche de Lima son las 05:00 UTC.
  return new Date(Date.UTC(a, m - 1, d + masDias, 12 - horaPeru)).toISOString();
}
...
export function normalizeModelo_(raw) {
  ...
  // Facturas a las que les falta el "X70" delante y solo dejaron la mecánica
  // ("1.5T 6MT 4X2 DELUXE"). Es la ficha del X70: ningún otro modelo del taller
  // se escribe como cilindrada + caja + tracción.
  if (/^\d[.]\d?T\s+\d?(MT|DCT|AT)\s+4X2/.test(s)) return "Jetour X70";
  ...
  if (/POLO/.test(s)) return "VW Polo";
  if (/\bGOL\b/.test(s)) return "VW Gol";
  if (/VIRTUS/.test(s)) return "VW Virtus";
  if (/TAOS/.test(s)) return "VW Taos";
  if (/T-?CROSS/.test(s)) return "VW T-Cross";
  if (/ARONA/.test(s)) return "Seat Arona";
  if (/IBIZA/.test(s)) return "Seat Ibiza";
  // RAMAL, DESCONOCIDO, vacíos y VINs pegados en la casilla del modelo se
  // quedan sin canónico a propósito: inventarles familia sería peor que
  // mostrarlos tal cual.
  return null;
}
...
/**
 * estadoGeneralDeConversion_ — el estado_general de una OT de CONVERSION a
 * partir de sus asignaciones.
 *
 * Una conversión la hacen dos, así que no está terminada hasta que lo están
 * MOTOR *y* TANQUE. La regla ya existía, pero escrita a mano dentro del handler
 * que registra eventos: el único sitio que la aplicaba era el técnico al
 * finalizar su parte. Cuando a un carro se le QUITABA un puesto, nadie la
 * volvía a evaluar y la OT se quedaba diciendo FINALIZADO con un puesto vacío —
 * un carro dado por terminado al que le faltaba media conversión, y sin nada
 * en pantalla que lo delatara.
 *
 * Las anuladas (`activo=false`) no cuentan: son trabajo que no ocurrió. Se
 * filtran aquí además de en el caller porque es justo el descuido que trajo
 * esta función al mundo.
 */
export function estadoGeneralDeConversion_(asignaciones = []) {
  ...
  if (motor === "FINALIZADO" && tanque === "FINALIZADO") return "FINALIZADO";
  return (motor || tanque) ? "EN PROCESO" : "PENDIENTE";
}
