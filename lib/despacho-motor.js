...
import { similitud_ as simPairing_ } from "./ml-pairing.js";
...
export function construirPool_({
  ...
  registrados = null,          // Set<vin> presentes en `vins` — null = no verificar
  ...
  for (const z of zonas) {
    ...
    // El VIN tiene que existir en `vins`: work_orders.vin es FK contra esa
    // tabla, así que proponer un carro no registrado no falla al asignarlo —
    // falla al CREAR la OT, con un 23503 que el motor reintentaría cada minuto
    // sin que el carro llegue nunca a nadie. Mejor excluirlo aquí y que el
    // motivo diga qué le falta: hay que registrar el VIN, no reparar el motor.
    //
    // Antes esto no hacía falta porque la lista diaria ya implicaba un VIN
    // registrado. Sin ese filtro, un carro estacionado sin registrar llega
    // hasta aquí (zonas 6 y 9 el 2026-08-14).
    if (registrados && !registrados.has(z.vin)) { excluidos[z.vin] = "VIN_NO_REGISTRADO"; continue; }
 ...
 *
 * Los pesos y la fórmula viven ahora en lib/ml-pairing.js, que es también lo
 * que usa routes/ml.js para entrenar y para la sugerencia que ve el técnico.
 * Se re-exportan desde aquí porque este módulo es la puerta pública del motor,
 * pero la definición es una sola: hasta hace poco había dos tablas de pesos
 * distintas y discrepaban en la "mejor pareja" de 5 de cada 10 técnicos.
 */
export { W_SIM } from "./ml-pairing.js";

/** Similitud entre dos user_id según el modelo entrenado. 0..1, 0.5 sin datos. */
function similitud_(idA, idB, techsPorId) {
  return simPairing_(techsPorId[idA], techsPorId[idB]);
}
