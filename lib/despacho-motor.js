...
export const PESOS = {
  familiaridad:   0.40,   // qué tan rápido es ESTA persona en ESTE modelo
  equidad:        0.30,   // quién va atrasado contra su meta del día
  cercania:       0.20,   // no cruzar el taller sin necesidad
...
export function construirPool_({
  ...
  for (const z of zonas) {
    ...
    if (!rolesLibres.length) { excluidos[z.vin] = "PUESTOS_OCUPADOS"; continue; }
...
/** Prioridad por atraso contra la meta del día. 0..1, 1 = no ha hecho nada. */
export function equidad_(unidad, creditosHoy = new Map(), meta = 2) {
  const ms = unidad.miembros || [];
  if (!ms.length) return 0.5;
  const prom = ms.reduce((s, m) => s + Number(creditosHoy.get(m.user_id) || 0), 0) / ms.length;
  return Math.min(1, Math.max(0, (meta - prom) / Math.max(1, meta)));
}
...
export function puntuar_(unidad, carro, ctx = {}) {
  const {
    indiceModelos = {}, creditosHoy = new Map(), metaCarros = 2,
    pesos = PESOS, parejaDe = null, techsPorId = {},
  } = ctx;

  const detalle = {
    familiaridad: familiaridad_(unidad, carro.modelo, indiceModelos),
    equidad:      equidad_(unidad, creditosHoy, metaCarros),
...
export function razonar_(detalle, carro, unidad) {
  ...
  if (detalle.equidad >= 0.75) {
    partes.push("va atrasado en su meta");
  }
