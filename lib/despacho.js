 ...
 *
 * Es la marca que sostiene la disolución: estas duplas viven y mueren con un
 * carro concreto, y una dupla ACTIVA saca a sus dos miembros del reparto. Si
 * alguna sobreviviera a su carro, el ayudante quedaría encadenado a él hasta el
 * fin de la jornada — la misma familia de bugs que la propuesta inmortal: nadie
 * lo nota hasta que alguien lleva tres horas sin carro.
 */
...
/** El VIN al que está atada una dupla automática. */
export function vinDeDuplaAuto_(dupla) {
  const m = /^AUTO_CARRO_EXTRA:([^\s]+)/.exec(String(dupla?.motivo || ""));
  return m ? m[1] : null;
}
...
export function pareoCarroExtra_({
  ...
  for (const d of duplasVivas) {
    // Las duplas DE TRABAJO (las que arma un técnico o el supervisor) no se
    // tocan: no cuelgan de ningún carro. Las automáticas sí.
    if (!esDuplaAuto_(d)) continue;
