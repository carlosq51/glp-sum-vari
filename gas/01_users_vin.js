...
function getUserEmailById_(userId) {
  const uid = String(userId || "").trim();
  if (!uid) return "";

  const ckey = `UID2EMAIL_${uid}`;
  const cached = cacheGetJson_(ckey);
  ...
  try {
    ...
    const data = shU.getRange(2, 1, lastRow - 1, shU.getLastColumn()).getValues();
    for (const row of data) {
      const id = String(row[colUUID - 1] || "").trim();
      if (id === uid) {
        const email = String(row[colEMAIL - 1] || "").trim().toLowerCase();
        cachePutJson_(ckey, { email }, 300);
        return email;
      }
    }
  } catch {}
